...
def triu_mask(y_mask, device, num_heads=8):
    """x: (xlen, ...)"""

    q_mask, k_mask = y_mask.T.unsqueeze(-1), y_mask.T.unsqueeze(1)

    zero_attn_mask = k_mask.data.eq(0)  # bx1xsk # mask zero-padding
    ...
    mask_label = list(set(mask_label))
    mask_label = [i for i in mask_label if i != 0]
    sent_attn_mask_total = torch.zeros(sent_attn_mat.size(), dtype=torch.int64,
                                       device=device).bool()  # mask sent-padding
    for i, label in enumerate(mask_label):
        for label2 in mask_label[i + 1:]:
            label_times = label * label2
            sent_attn_mask_eq = sent_attn_mat.eq(label_times)
            sent_attn_mask_total += sent_attn_mask_eq

    triu_attn_mask = torch.triu(torch.ones(sent_attn_mat.size(), device=device), 1).bool()
...
def kv_mask(q_mask, k_mask, device, num_heads=8):
...
class Transformer(NN):
    ...
    def forward_batch(self, x, x_mask, y, y_mask):
        # used for training
        x = self.src_emb(x)
        encode_mask = kv_mask(x_mask, x_mask, self.device)  #
        encode_out = self.encoder(x, encode_mask)  #
        decode_triu_mask = triu_mask(y_mask, self.device)
        decode_mask = kv_mask(y_mask, x_mask, self.device)  #
        y = self.trg_emb(y)
        decode_out = self.decoder(y, encode_out, decode_triu_mask, decode_mask)  # 12
