...
class Trainer:
    ...
    def fit(self,
        ...
        with tqdm(
            ...
            for _ in self.epoch_tqdm:
                ...
                self.train_epoch(model, train_dataloader)  # TODO: schuffle between epoches
                # torch.cuda.empty_cache()
    ...
    def forward_batch(self, model, batch):
        ...
        y_pred = model.forward_batch(x, x_mask, y, y_mask)
        return y_pred

    def calc_loss(self, y_pred, batch):
        sample_num = batch.sample_num
        y_gt = batch.trg_tensor[1:]
        loss = self.label_smoothed_nll_loss(y_pred, y_gt, sample_num[0])
    ...
    def label_smoothed_nll_loss(self, output, target, ture_sample_num):
        lprobs = output.log_softmax(-1)
        epsilon = 0.0  # 0.1
        ignore_index = self.pad_id
        reduce = True
        # calc loss
        fake_sample_num = int(target.size(-1))
        ratio = fake_sample_num / ture_sample_num
...
def main(
    ...
    trainer.fit(
        transformer,
        dataset.train_dataloader(train_batch_size, transformer.device),
        dataset.valid_dataloader(valid_batch_size, transformer.device),
        dataset.test_dataloader(test_batch_size, transformer.device)
...
if __name__ == '__main__':
    ...
    # data args
    parser.add_argument('--token_pack', type=str, default='source')  # source target
    parser.add_argument(
        '--src_lang',
        type=str,
        default='en',
