...
class Dataset:
    ...
    def bin_packing(self, df, src_lens, trg_lens, batch_groups):
        if self.token_pack == 'source':
            print('cat source token')
            token_lens = src_lens
        else:
            token_lens = trg_lens
            print('cat target token')
        ...
        for group in batch_groups:
            token_lens_dict = dict(token_lens[group])
            packs = binpacking.to_constant_volume(token_lens_dict, max(token_lens_dict.values()))
            batch_idx = []
            for pack in packs:
                ...
                src_mask = self.merge_sent([len(sen) for sen in src_sent])
                trg_mask = self.merge_sent([len(sen) for sen in trg_sent])
        ...
        new_df = pd.DataFrame(columns=[self.src_tok.lang, self.trg_tok.lang, 'src_mask', 'trg_mask'], data=new_df_list)

        return new_df, batch_idxs

    def merge_sent(self, lens):
        mask_label = [1, -1, 2, -2, 3, -3, 5, -5, 7, -7, 11, -11, 13, -13, 17, -17, 19, -19, 23, -23]
        mask_label = mask_label[:len(lens)]
        mask = []
        for i, l in enumerate(lens):
            mask.extend([mask_label[i]] * l)
        return mask
    ...
    def train_dataloader(self, batch_size, device):
        ...
        self.sample_num = [len(s) for s in batch_groups]
    ...
    def valid_dataloader(self, batch_size, device):
        ...
        batch_idxs = self.generate_groups(src_lens, trg_lens, batch_size)
        return torch.utils.data.DataLoader(
            df.values,
            batch_sampler=batch_idxs,
            collate_fn=collate_fn(device, self),
...
class collate_fn:
    ...
    def __call__(self, values):
        ...
        return Batch(self.dataset.src_tok.lang, src, src_mask, self.dataset.trg_tok.lang,
                     trg, trg_mask, self.dataset.sample_num)


class Batch:
    __slots__ = ['src_lang', 'src_tensor', 'src_mask', 'trg_lang', 'trg_tensor', 'trg_mask', 'sample_num']

    def __init__(self, src_lang, src_tensor, src_mask, trg_lang, trg_tensor, trg_mask, sample_num):
