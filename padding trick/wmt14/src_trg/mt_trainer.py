...
class Trainer:
    ...
    def fit(self,
        ...
        with tqdm(
            ...
            for _ in self.epoch_tqdm:
                ...
                self.train_epoch(model, train_samples, train_dataloader)  # TODO: schuffle between epoches
                # torch.cuda.empty_cache()
    ...
    def train_epoch(self, model, train_samples, train_dataloader):
        model.train()
        with tqdm(train_dataloader,
                  dynamic_ncols=True,
                  desc=f'train {self.epoch_tqdm.n}') as self.train_tqdm:
            for i, batch in enumerate(self.train_tqdm):
                self.train_batch(model, train_samples[i], batch)

                if self.should_stop:
                    break

    def train_batch(self, model, train_sample, batch):
        # cross-entropy minimization
        y_pred = self.forward_batch(model, batch)
        loss, acc = self.calc_loss(y_pred, train_sample, batch)
    ...
    def calc_loss(self, y_pred, batch_num, batch):
        y_gt = batch.trg_tensor[1:]
        loss = self.label_smoothed_nll_loss(y_pred, y_gt, batch_num)
    ...
    def label_smoothed_nll_loss(self, output, target, sample_num):
        ...
        if reduce:
            ...
            nll_loss = nll_loss * (target.size(1) / sample_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--token_packed', type=str,
                        default='source-target')  # source  or target or source-target or max
    parser.add_argument(
        '--src_lang',
        type=str,
        default='en',
