...
class Dataset:
    ...
    def generate_groups(self, src_lens, trg_lens, batch_size):
        ...
        for i, (sl, tl) in enumerate(zip(src_lens, trg_lens)):
            ...
            if len(batch_idx) * src_max > batch_size or len(batch_idx) * trg_max > batch_size:
                real_token_nums.append(len(batch_idx[:-1]) * max(src_max, trg_max))
                batch_idxs.append(batch_idx[:-1])
                batch_idx = batch_idx[-1:]
                src_max = sl
                trg_max = tl
        batch_idxs.append(batch_idx)
        real_token_nums.append(len(batch_idx) * max(src_max, trg_max))
        return batch_idxs, real_token_nums
    ...
    def train_dataloader(self, batch_size, device):
        ...
        batch_groups, real_token_nums = self.generate_groups(src_lens, trg_lens, batch_size)
        ...
        return real_token_nums, torch.utils.data.DataLoader(
            df.values,
