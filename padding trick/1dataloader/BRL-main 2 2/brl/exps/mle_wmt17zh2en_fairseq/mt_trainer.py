...
class Trainer:
    ...
    def train_batch(self, model, sample_num, batch):
        # cross-entropy minimization
        y_pred = self.forward_batch(model, batch)
        loss, acc = self.calc_loss(y_pred, sample_num, batch)
    ...
    def calc_loss(self, y_pred, sample_num, batch):
        y_gt = batch.trg_tensor[1:]
        loss = self.label_smoothed_nll_loss(y_pred, y_gt, sample_num)
