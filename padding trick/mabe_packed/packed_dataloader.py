...
class Dataset:
    ...
    def read_id(self, src_fname, trg_fname):
        ...
        src = list(map(lambda s: list(map(int, [self.src_tok.bos_id] + s.split() + [self.src_tok.eos_id])), src[:20]))
        trg = list(map(lambda s: list(map(int, [self.src_tok.bos_id] + s.split() + [self.src_tok.eos_id])), trg[:20]))
