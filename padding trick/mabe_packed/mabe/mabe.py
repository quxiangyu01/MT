...
#from brl.models.transformer_longtu import *
...
class Trainer:
    ...
    def train_bpbatch(self, model, batch):
        ...
        if self.num_bpbatchs % self.batches_per_step == 0:
            for pg in self.optim.param_groups:
                pg['lr'] = lr  #* (25000 / self.bs)  #* (self.bucket_size * self.accum_interval / self.bs)  # compensate lr by the bucket/batch size ratio
            self.optim.step()
    ...
    def lamin_loss(self, Q, z):
        ...
        Qt = Q[:-1].gather(dim=-1, index=z.unsqueeze(-1)).squeeze(-1)  # Qt[t-1][i] gives Q_t^i, for 1<=t<=L+1, 1<=i<=sent_num
