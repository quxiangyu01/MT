...
from brl.models.transformer_xiangyu import *
...
class Trainer:
    ...
    def fit(self,
        ...
        with tqdm(
            ...
            for _ in self.epoch_tqdm:
                ...
                self.train_epoch(model, train_real_token_nums, train_dataloader)  # TODO: schuffle between epoches
                # torch.cuda.empty_cache()
    ...
    def train_epoch(self, model, real_token_nums, train_dataloader):
        model.train()
        with tqdm(train_dataloader,
                  dynamic_ncols=True,
                  desc=f'train {self.epoch_tqdm.n}') as self.train_tqdm:
            for i, batch in enumerate(self.train_tqdm):
                self.train_batch(model, real_token_nums[i], batch)

                if self.should_stop:
                    break

    def train_batch(self, model, real_token_num, batch):
        # cross-entropy minimization
        y_pred = self.forward_batch(model, batch)
        loss, acc = self.calc_loss(y_pred, real_token_num, batch)
    ...
    def calc_loss(self, y_pred, real_token_num, batch):
        y_gt = batch.trg_tensor[1:]
        loss = self.label_smoothed_nll_loss(y_pred, y_gt, real_token_num)
    ...
    def label_smoothed_nll_loss(self, output, target, real_token_num):
        ...
        # ratio
        ratio = (target.numel() / real_token_num)
        ...
        if reduce:
            ...
            nll_loss = nll_loss * ratio
            smooth_loss = smooth_loss.mean()
            smooth_loss = smooth_loss * ratio
...
def main(
        ...
        src_train_fname='../../envs/wmt17zh2en_fairseq/train.clean.zh.id',
        trg_train_fname='../../envs/wmt17zh2en_fairseq/train.clean.en.id',
        # model args
        dim=512,
        num_layers=6,
        num_heads=8,
        ...
        need_test=True,
        src_valid_fname='../../envs/wmt17zh2en_fairseq/valid.clean.zh.id',
        trg_valid_fname='../../envs/wmt17zh2en_fairseq/valid.clean.en.id',
        src_test_fname='../../envs/wmt17zh2en_fairseq/test.zh.id',
        trg_test_fname='../../envs/wmt17zh2en_fairseq/test.en.id',
    ...
    dataset = Dataset(
        ...
        token_packed=token_packed,
    )
    dataset.setup(True, True, True)

    train_real_token_nums, train_loader = dataset.train_dataloader(train_batch_size, transformer.device)
    valid_real_token_nums, valid_loader = dataset.valid_dataloader(valid_batch_size, transformer.device)
    test_real_token_nums, test_loader = dataset.test_dataloader(test_batch_size, transformer.device)
    ...
    trainer.fit(
        transformer,
        train_real_token_nums,
        valid_real_token_nums,
        test_real_token_nums,
        train_loader,
        valid_loader,
        test_loader
...
if __name__ == '__main__':
    ...
    # data args
    parser.add_argument('--token_packed', type=str,
    ...
    main(
        ...
        seed=args.seed,
        token_packed=args.token_packed
    )
