...
class Dataset:
    ...
    def bin_packing(self, df, src_lens, trg_lens, batch_groups):
        def merge_sent(lens):
            mask_token = [1, -1, 2, -2, 3, -3, 5, -5, 7, -7, 11, -11, 13, -13, 17, -17, 19, -19, 23, -23, 29, -29, 31,
                          -31, 37, -37, 41, -41, 43, -43, 47, -47, 59, -59, 61, -61, 67, -67, 71, -71, 73, -73, 79, -79,
                          83, -83, 89, -89, 97, -97]
            mask = []
    ...
    def train_dataloader(self, batch_size, device):
        ...
        batch_idxs, samples = self.generate_groups(src_lens, trg_lens, batch_size)

        if self.packed_type is not None:
            # bin packing
            df, batch_idxs = self.bin_packing(df, src_lens, trg_lens, batch_idxs)
    ...
    @staticmethod
    def pad(idss, tok, device='cpu'):#, idss_mask
        """bos, eos, pad"""
...
class collate_fn:
    ...
    def __call__(self, values):
        src_idss, trg_idss = list(zip(*values)) #src_mask, trg_mask
        src = self.dataset.pad(src_idss, self.dataset.src_tok, self.device)#, src_mask
        trg = self.dataset.pad(trg_idss, self.dataset.trg_tok, self.device)#, trg_mask
