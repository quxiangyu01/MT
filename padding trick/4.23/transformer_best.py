...
def triu_mask(y_mask, num_heads, device):
    """x: (xlen, ...)"""
...
def kv_mask(q_mask, k_mask, num_heads, device):
...
def k_mask(x, padding_idx, num_heads):
    """x: (xlen, b)"""
    xlen = x.size(0)
...
class Transformer(NN):
    ...
    def forward_batch(self, x, y):
        ...
        if self.packed_type is not None:
            ...
            encode_mask = kv_mask(mask[0], mask[0], self.num_heads, x.device)
            decode_triu_mask = triu_mask(mask[1], self.num_heads, x.device)
            decode_mask = kv_mask(mask[1], mask[0], self.num_heads, x.device)
        else:
            #encode_mask, decode_triu_mask, decode_mask = None, None, None
            encode_mask = k_mask(x, self.pad_id, self.num_heads)
            decode_triu_mask = t_mask(y)
        ...
        def get_masks(x_list, last=False):
            ...
            for x in x_list:
                ...
                for i, e in enumerate(x):
                    if int(e) == self.bos_id:
                        temp_2.append(i)
                    elif int(e) == self.eos_id:
                        temp_3.append(i)
